...
예시:
에러: .all()을 "each item" 모드에서 사용
해결: "Run Once for All Items"로 변경

**절대 금지**:
- 4줄 이상 작성
- "현재 상태", "원인", "배경 설명" 같은 섹션
- "~일 수 있습니다", "~가능성" 같은 추측
- 여러 해결 방법 나열
- 장황한 설명

**위 형식대로 2-3줄로만 답변하세요. 이것은 매우 중요한 요구사항입니다.**
...
**에러 정보**:
${context.errorAnalysis.errors.map((err) => `- ${err.type}: ${err.message} (${err.nodeName})`).join('\n')}

위 에러를 2-3줄로 간단히 분석하세요.
...
짧고 명확하게, N8N 워크플로우만 답변하세요.${context.errorAnalysis ? `

🚨🚨🚨 다시 한 번 강조: 에러 분석은 반드시 2-3줄만! 🚨🚨🚨
형식: 에러: [한 줄] / 해결: [한 줄]
원인 설명 불필요. 해결 방법만 제시하세요.` : ''}`;
